<div class="text-white">
    <h3 class="text-center text-lg font-semibold mb-3">T. Cor.</h3>
    ...
    <!-- Lista de T. Cor. -->
    <div class="space-y-2">
        <h4 class="text-sm font-medium text-gray-300 mb-2">T. Cor. Registrados:</h4>
        @foreach($tcors as $item)
            <div class="flex justify-between items-center p-3 bg-gray-800 rounded-lg">
                <span class="text-white text-sm">{{ $item->tcor }}</span>
                <div class="flex space-x-2">
    ...
    <script>
        function confirmDeletion(id) {
            if (confirm('¿Está seguro de que desea eliminar este T. Cor.?')) {
                @this.deleteTcor(id);
            }
        }
    </script>
