<div>
    <h3>Nivel de Correspondencia</h3>
    <div class="flex items-center space-x-2 mb-2">
        <input type="text" wire:model.defer="newNcor"
               id="ncs"
    ...
    <ol class="list-none list-inside text-xs pt-2">
        @foreach($nncor as $items)
            <li class="flex justify-between items-center">
                <span>- {{ $items->ncor }} </span>
                <div>
{{--                    <button wire:click="startEdit({{ $items->id }})"--}}
{{--                            class="px-2 py-1 m-0.5 text-xs font-medium text-white bg-yellow-600 rounded-lg hover:bg-yellow-700">--}}
{{--                        Editar--}}
{{--                    </button>--}}
                    ...
                    <button wire:click="deleteNcor({{ $items->id }})"  {{-- red-orange--}}
                            class="relative inline-flex items-center justify-center p-0.5 mb-2 me-2
                                   overflow-hidden text-sm font-medium text-gray-900 rounded-lg group
