<div class="text-white">
    <h3 class="text-center text-lg font-semibold mb-3">Clas. Cor.</h3>
    ...
    <div class="mb-4">
        <div class="flex flex-col space-y-3">
            <input type="text" 
                   id="ccor-input" 
                   name="ccor" 
                   wire:model.defer="newCcor"
                   placeholder="Ingrese Clas. Cor."
    ...
    <!-- Lista de Clas. Cor. -->
    <div class="space-y-2">
        <h4 class="text-sm font-medium text-gray-300 mb-2">Clas. Cor. Registrados:</h4>
        @foreach($ccors as $item)
            <div class="flex justify-between items-center p-3 bg-gray-800 rounded-lg">
                <span class="text-white text-sm">{{ $item->ccor }}</span>
                <div class="flex space-x-2">
    ...
    <script>
        function confirmDeletion(id) {
            if (confirm('¿Está seguro de que desea eliminar este Clas. Cor.?')) {
                @this.deleteCcor(id);
            }
        }
    </script>
